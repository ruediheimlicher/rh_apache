...
my $TEMPERATURDATEI;
my $TEMPERATURDATEN;
my $SOLARTEMPERATURMITTEL;
my $PUMPEDATEN;
my $PUMPEDATEI;
my $TAGERTRAGDATEI;
my $SOLARDATEI;
my $SOLARDIAGRAMMDATEI;
my $HOMETEMPERATURDATEN;
my $SOLARPUMPEERTRAG;
my $STATUSFILE;
...
print '<html>
<head>
<title>solar_exp</title>
</head>
<body>
<h2>solar_exp</h2>';

# Fuer reinen Text: Content-type text/plain
...
#open LOGFILE, ">> $PWD/../Data/SolarLog.txt" || die "Z 80 Nicht einmal SolarLog gefunden\n";
...
# Last Time laden
open $PREFS, "<../Data/SolarCentralPrefs.txt" || die "SolarCentralPrefs.txt nicht gefunden\n";
...
if($anzZeilen == 0) # leeres File
{
	...
	open $TIMEPREFS, ">../Data/SolarTimePrefs.txt" || die "SolarTimePrefs nicht gefunden\n";
	print $TIMEPREFS time();
	close $TIMEPREFS;


	open $LOGFILE, ">>../Data/SolarLog.txt" || die " Auch dieses Logfile nicht gefunden\n";
...
else # Prefs schon vorhanden, aktualisieren
{
	...
	open $TIMEPREFS, "<../Data/SolarTimePrefs.txt" || die "SolarTimePrefs 4 nicht gefunden\n";
	@prefzeit = <$TIMEPREFS>;
	printf "TimePrefs: $prefzeit[0]<br>";
	$anzPrefsZeilen=@prefzeit;
	if ($anzPrefsZeilen == 0)
	{
		open $LOGFILE, ">>../Data/SolarLog.txt" || die "SolarLogfile nicht gefunden\n";
...
#
printf ("aktueller Datumstring: $Datumstring<br>");
...
#my $testnewPfad="../Data/SolarDaten/$oldJahrlong/SolarDaten$oldJahr$oldMonat$oldTag.txt";
#printf "newPfad: $testnewPfad<br>";

#	neuer Tag
my $newTag = "$tag";

#open LOGFILE, ">>../Data/SolarLog.txt" || die "newTag: Logfile nicht gefunden\n";
...
if ($oldTag ne $newTag) # Tagzahl hat sich geaendert
{
	...
	open $LOGFILE, ">>../Data/SolarLog.txt" || die "oldTag ne newTag: Logfile nicht gefunden\n";
	print $LOGFILE "solar_exp: neuer Tag: Laufzeit: $laufzeit  $Datumstring\n";
	close($LOGFILE);

	my $oldPfad="../Data/SolarDaten_exp.txt";
	#my $newFilename="SolarDaten/SolarDaten$jahr$monat$tag.txt";
	#my $newFilename="SolarDaten/SolarDaten_exp.txt";
	#my $newPfad="SolarDaten/SolarDaten$tag$stunde.txt";
	
	#my $newPfad="SolarDaten/SolarDaten$jahr$monat$tag.txt";
	my $newPfad="../Data/SolarDaten/$oldJahrlong/SolarDaten$oldJahr$oldMonat$oldTag.txt";
		
	#copy($oldPfad, $newPfad) or die "Copy failed: $!";
	#print PREFS "\noldPfad: $oldPfad newPfad: $newPfad\n";
	#close(PREFS);
	#open OLD,"<../Data/SolarDaten_exp.txt";
	open $OLD,"<$oldPfad";
	my @Data=<OLD>;
	...
	# Neu: JahrOrdner		
	my $newJahrOrdnerPfad="../Data/SolarDaten/$jahrlong/SolarDaten$oldJahr$oldMonat$oldTag.txt";
	...
	# Solardiagrammdaten
	my $oldDiagrammPfad="../Data/solardiagrammdaten.txt";
	my $newDiagrammPfad= "../Data/SolardiagrammDaten/$oldJahrlong/solardiagrammdaten$oldJahr$oldMonat$oldTag.txt";
	open $OLD,"<$oldDiagrammPfad";
	my @DiagrammData=<OLD>;
	
	open $NEW,">$newDiagrammPfad" || die "NEW nicht gefunden\n";
	print $NEW "@DiagrammData";
	close $NEW;
	open $OLD,">$oldDiagrammPfad" || die "OLD nicht gefunden\n";
	print $OLD "";
	close $OLD;
	...
	#
	 open $TAGELEKTROZEIT, "<../Data/ElektroDaten.txt" || die "TAGELEKTROZEIT nicht gefunden\n";
	...
	{	# Namespace fuer $TagPumpezeit
		 open $TAGPUMPEZEIT, "<../Data/SolarPumpeDaten.txt" || die "TAGPUMPEZEIT nicht gefunden\n";
		...
		#open $LOGFILE, ">>../Data/SolarLog.txt" || die "Pumpezeit des Tages lesen: Logfile nicht gefunden\n";
		#print $LOGFILE "Pumpezeit des Tages lesen: Pumpezeitstring: $Pumpezeitstring\n";
		#close($LOGFILE);
	
		open $PUMPEZEIT, ">>../Data/SolarPumpeZeit.txt" || die "PumpeZEIT nicht gefunden\n";
	...
	#
	 ...
	 open $ELEKTROZEIT, ">>../Data/ElektroZeit.txt" || die "ELEKTROZEIT nicht gefunden\n";
	...
	open $ELEKTRODATEI, ">../Data/ElektroDaten.txt" || die "ELEKTRODATEI nicht gefunden\n";
	...
	open $TEMPERATURDATEI, "<../Data/SolarTemperaturDaten.txt" || die "Solar TEMPERATURDATEI nicht gefunden\n";
	@Temperaturdatei = <$TEMPERATURDATEI>;
	chop(@Temperaturdatei);
	my $anzSolarTempZeilen=@Temperaturdatei;
	printf  "anzSolarTempZeilen: $anzSolarTempZeilen<br>";
	...
	#
		...
		open $SOLARTEMPERATURMITTEL, "<../Data/SolarTemperaturMittel.txt" || die "Solar TEMPERATURDATEI nicht gefunden\n";
		...
		printf  "anzTemperaturMittelZeilen: $anzTemperaturMittelZeilen<br>";
		...
		#open LOGFILE,  ">>../Data/SolarLog.txt" || die " TEMPERATURDATEI Logfile nicht gefunden\n";
		#print LOGFILE "SolarTemperaturMittel neue Zeile anlegen: $newTag.$newMonat.$newJahr\n";
		#close LOGFILE;
		
		# Solartemperaturmittel: neue Zeile anlegen
		open $SOLARTEMPERATURMITTEL, ">>../Data/SolarTemperaturMittel.txt" || die "SolarTemperaturMittel nicht gefunden";
	...
	#
	#	SolarTemperaturdaten: Werte zuruecksetzen
	#
	...
	#	
	open $PUMPEDATEI, "<../Data/SolarPumpeDaten.txt" || die "Solar TEMPERATURDATEI nicht gefunden\n";
	...
	if ($anzPumpeZeilen )
	{
		...
		# Datei fuer lesen oeffnen
		open $TAGERTRAGDATEI, "<../Data/SolarTagErtrag.txt" || die "Solar TAGERTRAGDATEI nicht gefunden\n";
		...
		if ($anzErtragZeilen) # schon Zeile(n) vorhanden
		{
			...
			#open LOGFILE,  ">>../Data/SolarLog.txt" || die " TAGERTRAGDATEI Logfile nicht gefunden\n";
			#print LOGFILE "TagErtrag lesen: oldertrag: $oldertrag\n";
			#close LOGFILE;
			
			# Datei fuer schreiben oeffnen, Array wieder schreiben
			open $TAGERTRAGDATEI, ">../Data/SolarTagErtrag.txt" || die "TAGERTRAGDATEI nicht gefunden";
			foreach (@ErtragArray)
			{
		...
		open $TAGERTRAGDATEI, ">>../Data/SolarTagErtrag.txt" || die "SolarTagErtrag nicht gefunden";
...
open $SOLARDATEI, ">>../Data/SolarDaten_exp.txt" || die "SOLARDATEI nicht gefunden\n";

open $SOLARDIAGRAMMDATEI, ">>../Data/solardiagrammdaten.txt" || die "SOLARDIAGRAMMDATEI nicht gefunden\n";

@Statistik=stat($SOLARDATEI);
$size=$Statistik[7];

#open LOGFILE, ">>../Data/SolarLog.txt" || die "Logfile nicht gefunden\n";
#print LOGFILE "Die Datei SolarDaten_exp.txt ist $size Bytes gross.\n";
#close(LOGFILE);
my @homedatei=0;
...
#open LOGFILE, ">>../Data/SolarLog.txt" || die "Logfile nicht gefunden\n";
#print LOGFILE "Die Datei SolarDaten_exp.txt hat $anzSolardateiZeilen Zeilen.\n";
#close(LOGFILE);
...
{
	open $LOGFILE, ">>../Data/SolarLog.txt" || die "Logfile Solarlog nicht gefunden\n";
...
#
open $PUMPEDATEI, "<../Data/SolarPumpeDaten.txt" || die "PUMPEDATEI nicht gefunden\n";
@Pumpedatei = <$PUMPEDATEI>;
chomp(@Pumpedatei);
...
if ($anzPumpedateiZeilen == 0)	# File ist noch leer, Titel schreiben
{
	...
	$Pumpedatei[8] = 0;										# Startzeit der aktuellen Einschaltung
	$Pumpedatei[9] = sprintf("Kollektor-Vorlauf: %2.1f",0.0);
	$Pumpedatei[10] = sprintf("Kollektor-Ruecklauf: %2.1f",0.0);
	$Pumpedatei[11] = sprintf("Ertrag: %.1f",0.0);
	$Pumpedatei[12] = sprintf("Ertragsumme: %.2f",0.0);	
	$Pumpedatei[13] = sprintf("Mittelwert: %.2f",0.0);
...
if ($anzSolardateiZeilen==0)							# File ist noch leer, Titel schreiben
{
	...
	$jahr +=2000;
	print $SOLARDATEI "Startzeit: $jahr-$monat-$tag 00:00:$sec +0100\n";
...
{
	#
	# Daten von SolarDaten_exp.txt lesen
	#
	...
	open $LOGFILE, ">>../Data/SolarLog.txt" || die "Solarlog: Logfile nicht gefunden\n";
	print $LOGFILE "$stunde:$min  Daten cgivars: $laufzeit\t*$cgivars{d0}\t*$cgivars{d1}\t*$cgivars{d2}\t*$cgivars{d3}\t*$cgivars{d4}\t*$cgivars{d5}\t*$cgivars{d6}\t*$cgivars{d7}\n";
	print $LOGFILE "$stunde:$min  Daten solar: $laufzeit\td0: $h0\td1: $h1\td2: $h2\td3: $h3\td4: $h4\td5: $h5\td6: $h6\td7: $h7\n";
	close $LOGFILE;
	...
	if ($oldMinute ne $newMinute) # neue Minute, Temperaturen speichern
	{
		...
		open $HOMETEMPERATURDATEN, "<../Data/TemperaturDaten.txt" || die "SolarTemperaturdaten nicht gefunden";
	...
	#	Temperaturdaten von HomeCentral verarbeiten
		...
		open $TEMPERATURDATEN, "<../Data/SolarTemperaturDaten.txt" || die "SolarTemperaturdaten nicht gefunden";
		@TemperaturArray = <$TEMPERATURDATEN>;
		$anzTemperaturZeilen=@TemperaturArray;
		chomp(@TemperaturArray);
		#open LOGFILE, ">>../SolarData/Log.txt" || die "TEMPERATURDATEN Logfile nicht gefunden\n";
		...
		else
		{
			...
			my $tempTemperaturR=($h1)/2.0;														# Kollektor Vorlauf
			$TemperaturArray[5] = sprintf("Kollektor-Ruecklauf: %.1f",$tempTemperaturR);		#	Wert
			
			my $ertrag=($tempTemperaturV-$tempTemperaturR);										#	Ertrag einer Minute
		...
		open $TEMPERATURDATEN, ">../Data/SolarTemperaturDaten.txt" || die "Temperaturdaten nicht gefunden\n";
		...
		open $LOGFILE, ">>../Data/SolarData/Log.txt" || die "Solardaten Log nicht gefunden\n";
		#print $LOGFILE "Solar $Datumstring Kollektor-Vorlauf: %.1f $tempTemperaturV Kollektor-Ruecklauf: %.1f $tempTemperaturR\n";
		printf $LOGFILE "Solar $Datumstring Kollektor-Vorlauf: %.1f Kollektor-Ruecklauf: %.1f \n",$tempTemperaturV ,$tempTemperaturR;
		...
		{
			open $LOGFILE, ">>../Data/SolarLog.txt" || die "Solardatei: Logfile nicht gefunden\n";
	...
	#	Pumpedaten schreiben (Datum und Zeit jedesmal aktualisieren)
		...
		open $PUMPEDATEN, ">../Data/SolarPumpeDaten.txt" || die "Pumpedaten nicht gefunden\n";
		foreach (@Pumpedatei)
		{
			print $PUMPEDATEN "$_\n";
		}
	...
	#	if ($oldMinute % 5 == 0) # Testphase
		...
		if ($oldStunde ne $newStunde) # Stunde zu Ende
		{
			...
			open $SOLARPUMPEERTRAG, "<../Data/SolarTagErtrag.txt" || die "SolarTagErtrag nicht gefunden";
			...
			open $PUMPEDATEN, ">../Data/SolarPumpeDaten.txt" || die "SolarPumpeDaten nicht gefunden\n";
			foreach (@Pumpedatei)
			{
				print $PUMPEDATEN "$_\n";
			}
	...
	open $STATUSFILE, ">../Data/SolarStatus.txt" || die "SolarStatusfile nicht gefunden\n";
	...
	if ($laufzeit> 0 && $datacontrol>0) # Keine Nullen in HomeData, keine Null-Temperaturen
	{
		...
		if ($min%2 == 0)
		{
			if ($min > $olddiagrammminute ||  $min == 0) # Daten fuer diese Minute noch nicht geschrieben
			{
				$olddiagrammminute =  $min;
				print $SOLARDIAGRAMMDATEI "$jahr\t$monat\t$tag\t$stunde\t$min\t$h0\t$h1\t$h2\t$h3\t$h4\t$h5\t$h6\t$h7\n";
				#print SOLARDIAGRAMMDATEI "$laufzeit\t$h0\t$h1\t$h2\t$h3\t$h4\t$h5\t$h6\t$h7\n";
			}
		}
		...
		open $LAST, "+>../Data/LastSolarData.txt" || die "LastSolarData nicht gefunden\n";
		my @lastdatei = <$LAST>;
...
#		}
	...
	{
		open $LOGFILE, ">>../Data/SolarLog.txt" || die "Laufzeit ne 0: Logfile nicht gefunden\n";
		if ($laufzeit == 0)
		{
		...
		if ($datacontrol == 0)
		{
#			print LOGFILE "$Datumstring\tdatacontrol ist 0\t$laufzeit\t$h2\t$h3\t$h4\t$h5\t$h0\t$h1\t$h6\t$h7\n";
		}
...
#open LOGFILE, ">>../SolarData/Log.txt" || die "Logfile nicht gefunden\n";
