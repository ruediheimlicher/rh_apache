...
$Datumstring = "Datum: $tag.$monat.$jahr $stunde:$min";
	open my $LOGFILE, ">>../Data/Log.txt" || die "Logfile A nicht gefunden\n";
	print $LOGFILE "Start Datumstring: $Datumstring\n";
	close($LOGFILE);

open my $DEBUGFILE, "<../Data/debugstatus.txt" || die "debuglog nicht gefunden\n";
@debugstatusdatei = <$DEBUGFILE>;
chomp(@debugstatusdatei);
$anzstatus=@debugstatusdatei;
my $debugstatus=13;
if ($anzstatus>3)
{
	#my $debugzeile = $debugstatusdatei[0];
	my @debugzeilearray= split(" ",$debugstatusdatei[0]);
	$debugstatus = $debugzeilearray[1];
}
close ($DEBUGFILE);

open my $DEBUGLOGFILE, ">>../Data/debuglog.txt" || die "debugstatus nicht gefunden\n";

#if ($debugstatus)
{
 print $DEBUGLOGFILE "$Datumstring\thome.pl debugstatus: $debugstatus\n";
}
...
open my $ALARMDATEI, "<../Data/HomeAlarmDaten.txt" || die "HOMEDATEI nicht gefunden\n";
...
# open my $LOGFILE, ">>../Data/AlarmLog.txt" || die "Logfile nicht gefunden H\n";
# print $LOGFILE "Die Datei AlarmDaten.txt hat $anzAlarmdateiZeilen Zeilen: \n";
# close($LOGFILE);

#	open my $ALARMDATEI, ">>../Data/AlarmDaten.txt" || die "ALARMDATEI nicht gefunden\n";
...
{
	open $LOGFILE, ">>../Data/AlarmLog.txt" || die "Logfile nicht gefunden H\n";
...
open my $PREFS, "<../Data/HomeCentralPrefs.txt" || die "Prefs 1 nicht gefunden\n";
...
# Wenn Prefs leer sind: neue Datei
#if ($prefsize == 0)

if($anzZeilen == 0)
{
	# Datum in Prosa einsetzen
	open $PREFS, ">../Data/HomeCentralPrefs.txt" || die "Prefs 2 nicht gefunden\n";
	#print $PREFS "N $Datumstring";
	$lastDatumstring=$Datumstring;
	print $PREFS "$Datumstring*";
	close ($PREFS);
	
	# Datum als zahl time() einsetzen
	open my $TIMEPREFS, ">../Data/TimePrefs.txt" || die "TimePrefs nicht gefunden\n";
	print $TIMEPREFS time();
	close($TIMEPREFS);
	
	open $LOGFILE, ">>../Data/Log.txt" || die "Logfile nicht gefunden A\n";
	print $LOGFILE "neues File fuer TimePrefs: $Datumstring\n";
	close($LOGFILE);
...
else
{
	...
	open $PREFS, ">../Data/HomeCentralPrefs.txt" || die "Prefs 3 nicht gefunden\n";
...
#	print $PREFS "prefdoc linien: $linien Daten: \n@prefdok";
	...
	# Datum als zahl time() einsetzen
	open my $TIMEPREFS, "<../Data/TimePrefs.txt" || die "TimePrefs 4 nicht gefunden\n";
	@prefzeit = <$TIMEPREFS>;
...
#		$laufzeit = 0;
		#
		open $LOGFILE, ">>../Data/Log.txt" || die "Logfile nicht gefunden B\n";
		print $LOGFILE "TimePrefs sind leer, anzPrefszeilen == 0  $Datumstring Tagsekunde: $tagsekunde\n";
		close($LOGFILE);
	...
	else
	{
		...
		open $LOGFILE, ">>../Data/Log.txt" || die "Logfile nicht gefunden C\n";
...
my $testnewPfad="../Data/HomeDaten/$oldJahrlong/HomeDaten$oldJahr$oldMonat$oldTag.txt";
	
printf "newPfad: $testnewPfad<br>";

my $newTag = "$tag";

#open $LOGFILE, ">>../Data/Log.txt" || die "Logfile nicht gefunden D\n";
...
#if (1)
{
	printf ("neue Minute: $newMinute<br>");
	open $LOGFILE, ">>../Data/Log.txt" || die "Logfile nicht gefunden E\n";
	print $LOGFILE "neuer Tag: laufzeit\n";
	close($LOGFILE);

	open $PREFS, ">../Data/HomeCentralPrefs.txt" || die "Prefs 226 nicht gefunden\n";
	print $PREFS "";
	close ($PREFS);
	...
	my $newPfad="../Data/HomeDaten/$oldJahrlong/HomeDaten$oldJahr$oldMonat$oldTag.txt";
	
	
	#copy($oldPfad, $newPfad) or die "Copy failed: $!";
	#print $PREFS "\noldPfad: $oldPfad newPfad: $newPfad\n";
	#close($PREFS);
	#open OLD,"<../Data/HomeDaten.txt" ;
	open my $OLD,"<$oldPfad" ;
	my @Data=<$OLD>;
	...
	# Neu: JahrOrdner		
	my $newJahrOrdnerPfad="../Data/HomeDaten/$jahrlong/HomeDaten$oldJahr$oldMonat$oldTag.txt";
	printf "$newJahrOrdnerPfad<br>";
	open my $NEWJAHR, ">$newJahrOrdnerPfad" || die "NEWJAHR nicht gefunden\n";
	print $NEWJAHR "@Data";
	close($NEWJAHR);	
	# JahrOrdner
	...
	# Brennerzeit des Tages  lesen
	 open my $TAGBRENNERZEIT, "<../Data/BrennerDaten.txt" || die "TAGBRENNERZEIT nicht gefunden\n";
	...
	# File fuer Brennerzeit oeffnen
	 ...
	 open my $BRENNERZEIT, ">>../Data/Brennerzeit.txt" || die "BRENNERZEIT nicht gefunden\n";
	 print($BRENNERZEIT $Brennerzeitstring);
	 close ($BRENNERZEIT);
	
	# File loeschen
	unlink("../Data/BrennerDaten.txt");
	
	# leeres File fuer Brennerdaten erzeugen
	open my $LEERESFILE,">../Data/BrennerDaten.txt" ;
	close($LEERESFILE);
	chmod(0755,"../Data/BrennerDaten.txt");
	...
	# Temperaturdaten des Tages  lesen
	 open my $TEMPERATURDATEI, "<../Data/TemperaturDaten.txt" || die "TEMPERATURDATEI nicht gefunden\n";
	 ...
	 {
		 $Temperaturmittel = 10;
		 $TagTemperaturmittel = 10;
		 $NachtTemperaturmittel = 5;
	 }
	 close ($TAGBRENNERZEIT);
	...
	# File fuer Temperaturmittel oeffnen, Mittelwert eintragen
	 ...
	 open my $TEMPERATURDATEI, ">>../Data/TemperaturMittel.txt" || die "TEMPERATURDATEI nicht gefunden\n";
	 print($TEMPERATURDATEI $Temperaturstring);
	 close ($TEMPERATURDATEI);
	
	# File loeschen
	unlink("../Data/TemperaturDaten.txt");
	
	# leeres File fuer Temperaturdaten erzeugen
	open $LEERESFILE,">../Data/TemperaturDaten.txt" ;
	close($LEERESFILE);
	chmod(0755,"../Data/TemperaturDaten.txt");
...
open my $HOMEDATEI, ">>../Data/HomeDaten.txt" || die "HOMEDATEI nicht gefunden\n";

@Statistik=stat($HOMEDATEI);
$size=$Statistik[7];

#open LOGFILE, ">>../Data/Log.txt" || die "Logfile nicht gefunden F\n";
...
open my $BRENNERDATEI, "<../Data/BrennerDaten.txt" || die "BRENNERDATEI nicht gefunden\n";
@Brennerdatei = <$BRENNERDATEI>;
chomp(@Brennerdatei);
$anzBrennerdateiZeilen=@Brennerdatei;

open $LOGFILE, ">>../Data/Log.txt" || die "Logfile nicht gefunden G\n";
#print LOGFILE "Die Datei BrennerDaten.txt hat $anzBrennerdateiZeilen Zeilen.\n";
close($LOGFILE);


#open LOGFILE, ">>../Data/Log.txt" || die "Logfile nicht gefunden H\n";
#print LOGFILE "Die Datei HomeDaten.txt hat $anzHomedateiZeilen Zeilen.\n";
#close(LOGFILE);

open $HOMEDATEI, ">>../Data/HomeDaten.txt" || die "HOMEDATEI nicht gefunden\n";

open $BRENNERDATEI, ">../Data/BrennerDaten.txt" || die "BRENNERDATEI nicht gefunden\n";
...
open $LOGFILE, ">>../Data/Log.txt" || die "Logfile nicht gefunden I\n";
...
else
{
	...
	my $maxhexcontrol=0; # Warnung, wenn nur 0xFF kommt
	...
	# Bug 12.12.12: Temperatursensor Defekt
	if ($h4==0)
	{
		$h4 = 20;
	}
	...
	print $LOGFILE "Stoerung: datacontrol: $datacontrol\tmaxhexcontrol: $maxhexcontrol\n";
	close($LOGFILE);
	...
	{
		open $LOGFILE, ">>../Data/Log.txt" || die "Logfile nicht gefunden G\n";
		print $LOGFILE "Stoerung: maxhexcontrol: $maxhexcontrol\n";
		close($LOGFILE);
		exit;
	}
	...
	if ($datacontrol == 0)  # Fehler: Vorlauf, Ruecklauf und Aussen sind nie null
	{
		...
		open $LOGFILE, ">>../Data/Log.txt" || die "Logfile nicht gefunden G\n";
		print $LOGFILE "datacontrol nach if: $datacontrol\n";
		print $LOGFILE "$Datumstring \t$laufzeit\t$h2\t$h3\t$h4\t$h5\t$h0\t$h1\t$h6\t$h7\n";
		close($LOGFILE);
		...
		if ($Alarmstatus == 0) # 	Alarm geht erst los. Alarmstatus ist noch nicht gesetzt
		{
			...
			$alarmdatei[3] = "Status: $Alarmstatus";	# Zeile 3
			open $LOGFILE, ">>../Data/Log.txt" || die "Logfile nicht gefunden G\n";
			print $LOGFILE "Alarm: datacontrol ist null: $datacontrol\n";
			print $LOGFILE "$Datumstring \t$laufzeit\t$h2\t$h3\t$h4\t$h5\t$h0\t$h1\t$h6\t$h7\n";
			close($LOGFILE);
		...
		if ($Resetstatus == 0) # Reset ist noch nicht gesetzt
		{
			...
			my $subject='homecentralalarm_home';

			my $pushto='ruediheimlicher@dopushmail.com';

			
			my $mailzeile=0;
			open my $MAILDATEI, "<../Data/alarm_mail.txt" || die "MAILDATEI nicht gefunden\n";
...
# 			close(MAIL);
	...
	{
		open $LOGFILE, ">>../Data/Log.txt" || die "Logfile nicht gefunden G\n";
		#print $LOGFILE "datacontrol nach else: $datacontrol\n";
	...
	{
		open my $TEMPERATURDATEN, "<../Data/TemperaturDaten.txt" || die "Temperaturdaten nicht gefunden";
	...
	open $LOGFILE, ">>../Data/Log.txt" || die "Logfile nicht gefunden G\n";
#	print $LOGFILE "$Datumstring\th4: $h4\n";
	close($LOGFILE);

		if ($h4 ==0)
		{
	#open $LOGFILE, ">>../Data/Log.txt" || die "Logfile nicht gefunden G\n";
	#print $LOGFILE "h4: $h4 ist null\n";
	#close($LOGFILE);
		...
		if ($h4) # Data fuer Aussentemperatur ist nie Null
		{
			...
			# Aenderung 22.12.12: Korrektur ist 32, nicht 40
			my $tempTemperatur=($h4 - 32)/2;
			...
			open $TEMPERATURDATEN, ">../Data/TemperaturDaten.txt" || die "Temperaturdaten nicht gefunden\n";
	...
	open $LOGFILE, ">>../Data/Log.txt" || die "Logfile nicht gefunden J\n";
	...
	#open LOGFILE, ">>../Data/Log.txt" || die "Logfile nicht gefunden K\n";
...
#	my $h7 = hex($cgivars{d7});
	...
	open my $STATUSFILE, ">../Data/Status.txt" || die "Statusfile nicht gefunden\n";
	print $STATUSFILE "$Datumstring \t$laufzeit\t$h2\t$h3\t$h4\t$h5\t$h0\t$h1\t$h6\t$h7\n";
	#print $STATUSFILE "Daten:  *$laufzeit*\t$cgivars{d2}*\t$cgivars{d3}*\t$cgivars{d4}*\t$cgivars{d1}*\t$cgivars{d0}*\n";
	close($STATUSFILE);
	
	open $LOGFILE, ">>../Data/Log.txt" || die "Logfile nicht gefunden G\n";
	# "Daten:  %d\t%d\t%d\t%d\t%d\t%d\t%d\t0 \n",$laufzeit,$cgivars{d2},$cgivars{d3},$cgivars{d4},$cgivars{d1},$cgivars{d0};
	#print $LOGFILE "Daten:  *$laufzeit*\t$cgivars{d2}*\t$cgivars{d3}*\t$cgivars{d4}*\t$cgivars{d1}*\t$cgivars{d0}*\n";
	#print $LOGFILE "$Datumstring \t$laufzeit\t$h2\t$h3\t$h4\t$h5\t$h0\t$h1\t$h6\t$h7\n";
	close($LOGFILE);
	...
	if ($laufzeit> 0 && $datacontrol>0) # Keine Nullen in HomeData, keine Null-Temperaturen
	{
		...
		open my $LAST, "+>../Data/LastData.txt" || die "LastData nicht gefunden\n";
		my @lastdatei = <$LAST>;
	...
	#	{
	#		open LOGFILE, ">>../Data/Log.txt" || die "Logfile nicht gefunden L\n";
	...
	{
		open $LOGFILE, ">>../Data/Log.txt" || die "Logfile nicht gefunden M\n";
		if ($laufzeit == 0)
		{
		...
		if ($datacontrol == 0)
		{
		print $LOGFILE "$Datumstring\thome.pl datacontrol ist 0\t$laufzeit\t$h2\t$h3\t$h4\t$h5\t$h0\t$h1\t$h6\t$h7\n";
		}
...
{
	open $ALARMDATEI, ">../Data/HomeAlarmDaten.txt" || die "Alarmdaten nicht gefunden\n";
	foreach (@alarmdatei)
	{
		print $ALARMDATEI "$_\n";
	}
...
#open LOGFILE, ">>../Data/Log.txt" || die "Logfile nicht gefunden N\n";
...
sub getcgivars {
   ...
   #     print LOGFILE "globvar: *$_*\n";
        if (!($_ eq ""))
        {
			...
			$in{$name}.= $value ;
	#		print LOGFILE "globvar sauber: *$_*\n";
        }
    }
   # my $cnt=%in;
    #print LOGFILE "globvar  anz: $cnt\n";
	close($LOGFILE);
