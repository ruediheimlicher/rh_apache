...
#open LOGFILE, ">>../public_html/Data/AlarmLog.txt" || die "AlarmLogfile A nicht gefunden\n";
#print LOGFILE "Alarm $Datumstring\tin_Variable: \t*$in_Variable*\n";
#close(LOGFILE);

#
printf ("Alarm $Datumstring<br>");
...
#	print LOGFILE "anzAlarmdateiZeilen ist OK\n";
	...
	if ($cgivars{d0})
	{
	$h0 = hex($cgivars{d0});	#  	Alarm von Tiefkuehltruhe und Wasser Keller, Estrich
...
#	print LOGFILE "Alarmbit h0: $h0\n";
	
	#	Die Kuehltruhe ist hops gegangen.
	#	Im Keller laeuft Wasser aus.
	#	Im Estrich laeuft Wasser aus.
	#	x
	#	Der Alarmstatus ist zurueckgesetzt.
	#	Der Bus ist blockiert.
	

	
	if ($h0 & 0x08) # Kuehltruhe ist hops
	{
		print LOGFILE "$Datumstring\^t";
		...
		{
			# Mail-Stuff
			my $title='HomeCentralAlarm_Kuehltruhe';
			my $to='r.heimlicher@bluewin.ch';
			my $from= 'ruediheimlicher@ruediheimlicher.ch';
			my $subject='homecentralalarm_kuehltruhe';
...
# 			close(MAIL);
	...
	{
		print LOGFILE "$Datumstring Alarm Kuehltruhe ist OK. ";
		printf "$Datumstring  Alarm Kuehltruhe ist OK.<br>";
	...
	if ($h0 & 0x10) # Wasser im Keller laeuft aus
	{
		...
		print LOGFILE "$Datumstring Alarm Wasser im Keller.\n";
		printf "$Datumstring  Alarm Wasser im Keller laeuft aus.<br>";
		...
		{
			# Mail-Stuff
			my $title='HomeCentralAlarm_Keller';
			my $to='r.heimlicher@bluewin.ch';
			my $from= 'ruediheimlicher@ruediheimlicher.ch';
			my $subject='homecentralalarm_wasser_keller';
...
#if ($writeTask) # AlarmDaten neu schreiben
{
...
#	print ERRLOGFILE "errCounter_Diff: $ErrDiff\t: $h9\n";
	...
	if ($anzfehler)
	{
		print ERRLOGFILE "Anzahl Fehler: $anzfehler";
	}
	else
	{
		print ERRLOGFILE "Keine Fehler";
	}
	print ERRLOGFILE "\n";
	close ERRLOGFILE;
...
sub getcgivars 
{
	open LOGFILE, ">>../public_html/Data/AlarmLog.txt" || die "Logfile nicht gefunden G\n";
