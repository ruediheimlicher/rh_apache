...
#
printf ("<h2>Stromlog</h2> <p>  $Datumstring</p>");
...
open LOGFILE, ">>../public_html/Data/TestDaten/testlog.txt" 
    or die("Cannot open file for writing");

#print LOGFILE "Test $Datumstring\n";


# Zeilen auf 1000 reduzieren
# Testdatei fuer Lesen oeffnen
open LOGFILE, "<../public_html/Data/TestDaten/testlog.txt" or die("Cannot open file for writing");

my @testdatei = <LOGFILE>;
...
close LOGFILE;
	...
	$h1 = ($cgivars{e1});							# 	error 1
		$datacontrol += $h1;
	$h2 = hex($cgivars{e2});							#	Boiler unten
		$datacontrol += $h2;
	$h3 = hex($cgivars{e3});							#	Boiler Mitte
		$datacontrol += $h3;
	$h4 = hex($cgivars{e4});							#	Boiler oben
		$datacontrol += $h4;
	$h5 = hex($cgivars{e5});							#	Kollektortemperatur
	
	$h6 = hex($cgivars{d6});							#	Solarstatus
	
	$h7 = hex($cgivars{d7});							# 	offen
	
	open LOGFILE, ">>../public_html/Data/TestDaten/testlog.txt" || die "StromLog: Logfile nicht gefunden\n";
...
#	print STROMMONITORDATEI "\n$Tagstring\t$stunde\t$min\t$sec\t$leistung";
	...
	#print LOGFILE "$Datumstring Daten cgivars: $laufzeit\t*$cgivars{d0}\t*$cgivars{d1}\t*$cgivars{d2}\t*$cgivars{d3}\t*$cgivars{d4}\t*$cgivars{d5}\n";	
	print LOGFILE "$Datumstring\tstromlog.pl Strom L:\t $leistung\tWs:\t$wattstunden\tpk:\t$paketcounter\tcb err:\t$h0\te1:\t$h1*\n";	#\td2: $h2\td3: $h3\td4: $h4\td5: $h5\n";
	close LOGFILE;


		#open LOGFILE, ">../public_html/Data/TestDaten/testlog.txt" || die "LOGFILE nicht gefunden\n";
		#	print LOGFILE "$Datumstring\tStrom L:\t $leistung\tWs:\t$wattstunden\tpk:\t$paketcounter\tcb err:\t$h0\te1:\t$h1*\n";	#\td2: $h2\td3: $h3\td4: $h4\td5: $h5\n";
